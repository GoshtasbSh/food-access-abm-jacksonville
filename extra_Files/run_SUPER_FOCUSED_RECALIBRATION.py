"""
SUPER FOCUSED RE-CALIBRATION
=============================
Uses previous best parameters and explores a VERY focused grid.

Previous Best (without pantries/delivery):
- α=2.5, β=0.7, γ=1.0, δ=0.4
- Thresholds: L=5.5, M=7.0, H=14.0

SUPER FOCUSED GRID: 27 configurations (3^3 - only vary 3 key parameters!)
ESTIMATED TIME: 5-8 minutes (Phase 1) + 15-20 minutes (Phase 2) = 20-28 minutes total
"""
...
if __name__ == "__main__":
    print("=" * 80)
    print("SUPER FOCUSED RE-CALIBRATION")
    print("=" * 80)
    print()
    print("Previous Best Parameters (without pantries/delivery):")
    print("  α=2.5, β=0.7, γ=1.0, δ=0.4")
    print("  Thresholds: L=5.5, M=7.0, H=14.0")
    print()
    print("Strategy: Only vary the 3 MOST IMPORTANT parameters")
    print("  • α (distance sensitivity)")
    print("  • β (price/budget sensitivity)")
    print("  • γ (quality/variety weight)")
    print()
    print("Fixed parameters:")
    print("  • δ=0.4 (convenience weight - previous best)")
    print("  • Thresholds: L=5.5, M=7.0, H=14.0 (previous best)")
    print()
    print("=" * 80)
    print()
    ...
    # SUPER FOCUSED: Only vary alpha, beta, gamma (3^3 = 27 configs)
    param_grid = {
        'alpha': [2.0, 2.5, 3.0],
        'beta': [0.6, 0.7, 0.8],
        'gamma': [0.8, 1.0, 1.2]
    ...
    print(f"🔍 PHASE 1: SUPER FOCUSED GRID SEARCH")
    print("=" * 80)
    ...
    for i, params in enumerate(combinations):
        ...
        print(f"[{i+1}/{len(combinations)}] α={params['alpha']:.1f}, β={params['beta']:.1f}, γ={params['gamma']:.1f} (δ=0.4 fixed)", end='', flush=True)
    ...
    phase1_file = f"SUPER_FOCUSED_PHASE1_{timestamp}.csv"
    ...
    print(df_phase1[['alpha', 'beta', 'gamma', 'avg_spend_low', 'avg_spend_med', 'avg_spend_high', 'corner_share', 'error']].head(5).to_string())
    print()
    
    # PHASE 2: Full validation of top 3 (faster!)
    print("=" * 80)
    ...
    phase2_file = f"SUPER_FOCUSED_PHASE2_{timestamp}.csv"
    ...
    final_params = {
        ...
        'mobile_pantries': [
            {'name': 'JaxPAL', 'schedule': '3rd Tuesday monthly', 'location': '3450 Monument Rd'},
            {'name': 'Bethany Ministries', 'schedule': '2nd Tuesday monthly', 'location': '8550 Arlington Expy'},
            {'name': 'Paxon Revival Center', 'schedule': '2nd & 5th Wednesday monthly', 'location': '5461 Commonwealth Ave'}
        ],
        'delivery_service': {
            'type': 'Market-Rate',
            'base_fee': 2.00,
    ...
    print(f"   δ (convenience) = {best['delta']:.2f} [FIXED]")
    print(f"   Thresholds: Low={best['threshold_low']:.1f}, Med={best['threshold_med']:.1f}, High={best['threshold_high']:.1f} [FIXED]")
    print("\n📊 FINAL RESULTS (mean ± std across 5 seeds):")
    print(f"   Annual Spend: Low=${best['avg_spend_low']:.0f}±{best['std_spend_low']:.0f} (target: $2,400)")
    print(f"                 Med=${best['avg_spend_med']:.0f}±{best['std_spend_med']:.0f} (target: $5,200)")
    print(f"                 High=${best['avg_spend_high']:.0f}±{best['std_spend_high']:.0f} (target: $8,800)")
    print(f"   Corner Share: {best['corner_share']:.1%}±{best['std_corner_share']:.2%} (target: 5%)")
    print(f"   Travel Distance: Car={best['avg_dist_car']:.2f}±{best['std_dist_car']:.2f}km (target: 6.5km)")
    print(f"                    No-car={best['avg_dist_nocar']:.2f}±{best['std_dist_nocar']:.2f}km (target: 2.0km)")
