...
def _load_calibrated_params_from_json():
    ...
    # Priority: FINAL_CALIBRATED_PARAMS > BEST_PHASE1_PARAMS > others.
    # Use mtime only as tiebreaker within the same priority tier.
    def _priority(f):
        name = _os.path.basename(f)
        tier = 0 if 'FINAL_CALIBRATED' in name else (1 if 'BEST_PHASE1' in name else 2)
        return (tier, -_os.path.getmtime(f))   # lower tier = higher priority
    latest = min(files, key=_priority)
    try:
...
class SimulationConfig:
    ...
    # Technical/spatial parameters (defaults from config; override via env vars)
...
class EnhancedHouseholdAgent(mg.GeoAgent):
    ...
    def step(self):
        ...
        if needs_to_shop:
            ...
            # Step 2: Consider delivery if household is a delivery user
...
class EnhancedMesaGeoModel(mesa.Model):
    ...
    def _load_road_network(self) -> Optional[nx.Graph]:
        """Load road network for enhanced routing. Not yet implemented; uses Euclidean distances."""
        try:
    ...
    def step(self):
        ...
        daily_metrics = {
            ...
            'spatial_equity_index': self._calculate_spatial_equity_index(),
            'total_revenue': self._calculate_total_revenue(),
            # Cumulative spending by income group (annualised at end of run)
            'spend_low':    _spend_by_income(self, 'low'),
            'spend_med':    _spend_by_income(self, 'medium'),
            'spend_high':   _spend_by_income(self, 'high'),
            # Store-type shares (cumulative trip fractions)
            'corner_share':   _corner_store_share(self),
            'pantry_share':   _pantry_share(self),
            'delivery_share': _delivery_share(self),
        }
